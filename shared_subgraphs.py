...
def qy_graph(x, k=10):
    ...
    with tf.variable_scope('qy'):
        ...
        # h3 = Dense(h2, 128, 'layer3', tf.nn.relu, reuse=reuse)
        # qy_logit = Dense(h3, k, 'logit', reuse=reuse)
        x = tf.reshape(x,[-1, 28, 28, 1])
        h1 = Conv2d(x, 16, [3,3], [1,1], scope = 'convlayer1', activation = tf.nn.relu, reuse = reuse)
        h1 = Max_pool(h1, [2,2], [1,1], scope = 'convlayer1')
        h2 = Conv2d(h1, 16, [3,3], [1,1], activation = tf.nn.relu, reuse = reuse, scope = 'convlayer2')
